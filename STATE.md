...
- **[进行中] 语音定制首页 (SDUI)** — 设计 + Phase 1 计划已提交在分支 `design/sdui-homepage`（spec `docs/superpowers/specs/2026-06-28-voicedrop-sdui-homepage-design.md`；计划 `docs/superpowers/plans/2026-06-28-sdui-homepage-phase1-ios-engine.md`，**计划顶部有「交接状态」**）。用户语音描述首页 → 生成 `users/<sub>/page.json` (SDUI 文档) → app 原生渲染；缺/坏则回退原生首页。Task 1（测试 target）已就位，Task 2–7 待在装了 iOS SDK 的 Mac 上接手。
