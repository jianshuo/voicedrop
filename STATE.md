...
- `users/<sub>/page.json` — 用户自定义首页（SDUI 文档）`{schema,version,root:<node>}`。缺失/损坏 → app 渲染原生默认首页（零回归）。词表（v1 封闭白名单）见 spec `docs/superpowers/specs/2026-06-28-voicedrop-sdui-homepage-design.md`。Phase 1 = iOS 渲染引擎（已完成，分支 `design/sdui-homepage`）；Phase 2 = 语音生成（PageEditor DO，未做）。
...
## 长按操作菜单（2026-07-04，worker 已上线，iOS 在 design/sdui-homepage）
...
- **iOS**：`UIConfigStore.swift`（Codable 模型 + 详情页出现时拉取 + UserDefaults 缓存 + 与服务端
  ...
  「正在改」/placeholder 全复用）。
- 部署状态：worker 已 deploy + 线上冒烟（401/200 + 菜单内容）；iOS commit `e4ec435`
  （design/sdui-homepage），**真机手测待做**（长按图→卡通→placeholder→出图；段落→更简洁；
  题图→顶部横幅；拷贝；制作中无菜单）。
...
- **SDUI 可定制首页（Phase 1，2026-07-03）**：`LibraryView` 现在是「固定外壳（topBar）+ (自定义页 ?
  `PageRenderer` : 原生首页)」。新文件 `PageModel.swift`（白名单节点 + 健壮解码，root type 不在词表→nil、
  字段非法→节点降级 `.unknown`）、`PageStore.swift`（拉 `download/page.json`，纯函数 `resolveTree`：404→原生、
...
- **[Phase 1 完成] 语音定制首页 (SDUI)** — 分支 `design/sdui-homepage`（2026-07-03 已 merge 当时的 main 再实现）：iOS 渲染引擎全部落地（Task 1–7，见「iOS app」段的 SDUI 条目），模拟器端到端验证过（自定义页渲染 / 坏 JSON 回退 / 删除回原生）。spec `docs/superpowers/specs/2026-06-28-voicedrop-sdui-homepage-design.md`；计划 `docs/superpowers/plans/2026-06-28-sdui-homepage-phase1-ios-engine.md`。**Phase 2（语音生成 page.json，PageEditor DO）未开始**——目前只能手动 `PUT upload/page.json` 定制。
