...
**Upload resilience (`Uploader.swift` + `BackgroundTransfer.swift`，2026-08-04 起后台会话):** the Documents dir IS the pending queue — a `VoiceDrop-*.m4a` still on disk = not yet uploaded (shows 正在上传)。音频与 tags 边车的 PUT 走 **background URLSession**（`BackgroundTransfer`，identifier `com.wangjianshuo.VoiceDrop.upload`）：录完立刻锁屏/切走/进程被杀，传输由系统守护进程接管完成；完成事件在进程复活后经 `PushRegistrar.application(_:handleEventsForBackgroundURLSession:)` 回放。收尾（删本地/keepLocal、清边车、乐观行、埋点）集中在 delegate → `Uploader.shared.finishAudioTransfer` **幂等**执行——进程一直活着（`upload()` 的 await 正常返回）和隔世重启两条路收敛到同一结果；Uploader 因此**单例化**（`Uploader.shared`）。**根因（2026-08-04）**：旧前台 `URLSession.shared` 只有 bg assertion ~30s 缓冲，第一枪没打完随挂起冻结、无人再试，文件要等「下次打开 App」的 drain 才补传——实测 8/1–8/3 每条录音的上传延迟恰好等于下次打开 App 的间隔（22 分钟～2 小时），与文件大小无关。**重试语义**：bg session 在 resource 窗口（6h）内自己等网自己重试，进程内的「3 次尝试+退避 sleep」已删除；服务器真拒绝（4xx/5xx）或窗口耗尽 → 文件留盘，下一次 drain 触发点（前台刷新/联网恢复/下次录音）重新入队（never lost）；同一文件按 `taskByPath` 去重，并发入队挂到在飞任务上等同一结果。drains 仍 serialised（`isDraining`/`drainAgain`），`NWPathMonitor` 联网重灌不变。⚠️ 用户 force-quit 会让系统取消后台任务——文件仍在盘上，下次启动重新入队；任务元数据在 `taskDescription`（`Job` JSON，存 Documents **相对**路径——容器绝对路径每次安装会变）。

**音频不再等照片/边车（2026-08-03，弱网提速）：** `upload()` 开头曾串行 await「tags 边车 PUT + 照片队列全 drain（每张 3 次退避重试）」——8-01 苏州弱网实测中位 **214s 且与文件大小无关**（线路 voicedrop.cn 1.7MB/s 无碍）的真凶。现在音频立刻 PUT：照片 drain fire-and-forget 并行赶路（晚到照片由服务端 Miner DO `backfillSessionPhotos` 兜底，见 R2 layout 照片节），tags 边车并行 Task（成功分支 `await tagsTask.value` 后删本地边车；消费点在成文时刻，几百字节永远先到）。`PhotoUploadQueue.runDrain` = `withTaskGroup` 并发 3（`maxConcurrent`）、每张每次 drain 只试一次、无退避 sleep，失败留盘交给下一次 drain 触发点（启动/联网恢复/enqueue/音频上传前/前台刷新）。多条待传音频仍串行（罕见态，第一条最早到达）；background URLSession 已于 2026-08-04 落地（见上面 Upload resilience 节）。埋点「录音上传完成/失败」2026-08-03 带 **`口径:2`**、2026-08-04 起带 **`口径:3`**（后台会话：`耗时秒`=入队→系统送达，**含挂起期**，即用户体感的「录完到传完」；`排队秒` 沿用口径 2 语义。PostHog 按口径过滤别混算）；照片侧「照片队列清空」`{张数,失败,耗时秒,并发}`。
...
## 网络线路 — 按 App Store 商店区域定入口（2026-08-25；竞速探测已移除）

iOS 的全部 HTTP API/资源主机由 **`APIRoute`**（`Networking.swift`）运行时决定：
**中国区商店账号 = `voicedrop.cn`（腾讯 EdgeOne 境内边缘）；其他区 = `jianshuo.dev`
（Cloudflare 直连）**。

- **判定 = StoreKit `Storefront.current.countryCode`**（用户 App Store 账号的商店区域，
  与 IP/系统语言无关；TestFlight 与 Xcode 直装读到的都是设备登录的商店账号区域）。
  `CHN` 或取不到（模拟器/未登录）→ voicedrop.cn；其他 → jianshuo.dev。主 App 启动取一次
  （`VoiceDropApp.swift` `noteStorefront`，PostHog 埋「商店区域」`{区域,线路}`），持久化
  App Group UserDefaults（`api.route.storefront`）——Share Extension 直接沿用。进程内
  `OSAllocatedUnfairLock` 缓存，URL 构建高频路径不反复开 UserDefaults。
- **历史**：2026-08-19~08-25 曾用「并发 HEAD 竞速 + 150ms 迟滞」自动选线（`probe/pick`，
  单测 APIRouteTests）；2026-08-25 按「判定简单可预期」原则整体移除，回滚点在 git 历史。
...
文本截 2000 字。iOS 入口 = 设置「其他」卡 `FeedbackSheet`。测试 `agent/test/feedback.test.js`。
**查看**：控制台页 `jianshuo.dev/voicedrop/admin/feedback`（后端 admin-only
`GET /files/api/feedback/{dates,list?date=}`，测试 `agent/test/feedback-admin.test.js`）或手机推送。

同卡「使用手册」行 = **内置 sheet**（2026-08-09 起，不再外跳网页）：`HelpManualView.swift`
的 `ManualParser` 把 bundle 资源 `Resources/HelpManual.md` 解析成块（章节/表格/代码/列表），
SwiftUI 按 Theme 原生排版，顶部章节 chips 跳转。**内容真源 = jianshuo.dev repo
`voicedrop/help/manual/manual.md`**，网页版改动后 `cp` 一份到 `Resources/HelpManual.md` 保持
...
- **App Store**: **1.11 在售**；**1.12 / build 318 已提审 `WAITING_FOR_REVIEW`（2026-08-18，AFTER_APPROVAL 过审即发布）——首个订阅版**：`monthly_19_9` 订阅 + 订阅组随 1.12 同提审单送审（订阅/组/版本三者都 WAITING_FOR_REVIEW）。**订阅已建全**（2026-08-18 用 ASC API 补齐：zh-Hans/en-US 本地化、175 地区 ¥19.9 等价价格、审核截图、reviewNote、availability→READY_TO_SUBMIT）。**首订阅 API 提审姿势（fastlane 做不了）**：deliver 提的审只含版本条目、订阅不会自动随行（sub 停在 READY_TO_SUBMIT 就没进审）——正确做法：cancel 提审 → `POST /v1/reviewSubmissions` 新草稿 → items 挂 appStoreVersion → `POST /v1/subscriptionVersions`（订阅拍送审快照，已存在会 409 ALREADY_EXISTS 并告知 id）→ items 再挂 `subscriptionGroupVersion`（组的 versions 端点查 id）+ `subscriptionVersion` → PATCH submitted:true。注意 fastlane 的 Spaceship 模型没有这两种 item 关系，要直接调 API。**售卖开关已开**（R2 `config/iap.json`={"enabled":true}，2026-08-18）：1.11 线上用户在过审前会看到订阅卡但购买会失败（商品未过审 StoreKit 加载不到），过审即自动可买；要临时藏卡就把 iap.json 写成 false。**仍欠一步手工**：ASC → App 信息 → App Store 服务器通知 V2（生产+沙盒 URL 填 `https://jianshuo.dev/agent/iap/notifications`，API 配不了；没配时续费发放走 App 启动 claim 兜底，不实时但不丢）。**1.6 提审踩坑（已解决）**：ASC 上有个 READY_FOR_REVIEW 的提交草稿装着**订阅组条目**（monthly_19_9 首次审核需搭版本），fastlane 拒绝（items not managed by fastlane）；且订阅本体是**空壳**（0 本地化/0 价格/无审核截图 → MISSING_METADATA），组无法过闸。处置：API 把订阅组条目从提交里摘除（DELETE reviewSubmissionItems）、把 fastlane 已备好的 1.6 版本加进同一提交（POST reviewSubmissionItems）、PATCH submitted:true。**订阅要上架还欠**：subscriptionLocalizations（zh-Hans 名称+描述）、订阅价格（¥19.9 pricePoint）、审核截图（付费墙 UI 实拍）、availability，补齐后随 1.7 一起提审（首个订阅必须搭版本）。ASC 隐私标签用户已手动更新（2026-07-15：标识符→用户 ID + 使用数据→产品交互，关联身份、用途=分析、不追踪）。提审流程：project.yml MARKETING_VERSION 垫到目标版本 + 更新双语 `fastlane/metadata/*/release_notes.txt` → push main（2026-08-02 起自动发 TestFlight，`[tf]` 闸已撤销）→ dispatch `appstore`（`fastlane release skip_build:true` 复用 TestFlight 已处理 build）→ 打 `release/<v>` tag 推远端（CI 里 lane 打的 tag 不会推回来）。(Resubmit = cancel review submission via ASC API `PATCH reviewSubmissions/{id} canceled:true`, then dispatch `appstore`.)
