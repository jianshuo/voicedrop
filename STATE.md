...
iOS app ──record──> users/<sub>/VoiceDrop-<ts>...m4a  (R2, per-user)
                          │
  GitHub Actions `mine` (dispatched on every new audio upload + 加急处理; no cron)
...
- Triggered on every new audio upload (files API auto-dispatch) + the 加急处理 button. No cron. Idempotent: skips anything with a `.json` or `.empty` marker.
