...
**音频不再等照片/边车（2026-08-03，弱网提速）：** `upload()` 开头曾串行 await「tags 边车 PUT + 照片队列全 drain（每张 3 次退避重试）」——8-01 苏州弱网实测中位 **214s 且与文件大小无关**（线路 voicedrop.cn 1.7MB/s 无碍）的真凶。现在音频立刻 PUT：照片 drain fire-and-forget 并行赶路（晚到照片由服务端 Miner DO `backfillSessionPhotos` 兜底，见 R2 layout 照片节），tags 边车并行 Task（成功分支 `await tagsTask.value` 后删本地边车；消费点在成文时刻，几百字节永远先到）。`PhotoUploadQueue.runDrain` = `withTaskGroup` 并发 3（`maxConcurrent`）、每张每次 drain 只试一次、无退避 sleep，失败留盘交给下一次 drain 触发点（启动/联网恢复/enqueue/音频上传前/前台刷新）。多条待传音频仍串行（罕见态，第一条最早到达）；background URLSession 仍是 future work。埋点「录音上传完成/失败」带 **`口径:2`**（`排队秒`=drain 里排在前面的音频占用的等待，单条恒≈0；口径 1 的照片串行等待已消亡，PostHog 按口径过滤别混算）；照片侧「照片队列清空」`{张数,失败,耗时秒,并发}`。
